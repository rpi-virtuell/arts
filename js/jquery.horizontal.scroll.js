...
function dw_scrollObj(wndoId, lyrId, horizId) {
	...
	if (wn.addEventListener) {
		...
		dw_scrollObj.mouseout = true;

//		jQuery('.author-info-container-wrapper').hover(
//				function(event) {
//					console.log(event.type)
//					if (event.type == 'mouseleave') {
//						dw_scrollObj.mouseout = true;
//					} else {
//						dw_scrollObj.mouseout = false;
//					}
//				});
...
dw_scrollObj.doubleSpeed = function(wndoId) {
	if (dw_scrollObj.col[wndoId])
		dw_scrollObj.col[wndoId].speed *= 2;
}
...
dw_Util.contained = function(oNode, oCont) {
	if (!oNode)
		return null; // in case alt-tab away while hovering (prevent error)
